...
**中英文技术类 Markdown 文档专业审查工具**

#### 作用

该 skill 用于严格校验中英文技术类 Markdown 文档，提供专业的 Code Review 级别的文档质量审查。它能够：

- 检查 Markdown 格式规范（标题层级、列表、代码块、表格、链接等）
- 识别中英文错别字、语法错误和口语化表达
- 验证技术术语统一性和中英文混排规范
- 评估文档结构逻辑和可读性
- 检测数值计算、公式验证、引用有效性等逻辑问题
- 发现段落完整性、章节结构一致性等深层问题

#### 使用场景

- **文档质量审查**：检查技术文档的格式规范和可读性
- **术语一致性检查**：确保同一术语在文档前后表达一致
- **错别字排查**：发现中英文拼写错误、语病和表达问题
- **格式规范化**：统一标题层级、列表格式、代码块标注等
- **技术文档评审**：评审 API 文档、技术手册、开发指南等

#### 在 opencode 中使用

该 skill 已注册为 opencode 的自动触发 skill。当用户需要 review Markdown 文档、检查技术文档质量、校验文档格式、查找错别字或术语不一致时使用此技能。

**触发方式**：

1. **直接请求审查**：告诉 opencode 需要审查的 Markdown 文件（如 `review 这个 Markdown 文件 README.md`）
2. **检查特定问题**：提出具体问题（如 `帮我检查这个文档的错别字`、`这个文档术语是否统一`）
3. **质量评估**：请求文档质量评分（如 `评估这个技术文档的质量`）

**opencode 会自动**：
- 读取并分析指定的 Markdown 文件
- 按照六遍审查流程进行深度检查
- 输出标准化的审查报告（包含评分、问题列表、修改建议）
- 提供典型修复示例（如适用）
...
用户输入：
```
帮我 review 这个技术文档 docs/api_guide.md
```

opencode 自动触发 skill，输出审查报告：
```
## 文档评分：7/10
...
### 技术规范与术语问题
1. API/apis 混用 - 建议统一为 "API"
2. HTTP/http 大小写不统一 - 建议统一为 "HTTP"

### 优化建议
1. 为代码块添加语言标注（如 ```bash）
2. 统一技术术语的大小写格式
3. 在中英文之间添加空格（如 "支持 HTTP 协议"）
...
### 发现的问题：
1. "URL" 和 "url" 混用 - 建议统一为 "URL"（共 3 处不一致）
2. "Config" 和 "Configuration" 混用 - 建议统一为 "Config"（共 2 处不一致）
3. "V1.0" 和 "v1.0" 混用 - 建议统一版本号格式

### 建议：
在文档开头添加术语表，明确所有技术术语的标准写法。
```
...
3. **第三遍**：检查术语统一性和中英文混排
4. **第四遍**：检查技术细节内部自洽性（命名与描述匹配）
5. **第五遍**：评估章节结构一致性和同构对比异常
6. **第六遍**：评估可读性和逻辑结构
